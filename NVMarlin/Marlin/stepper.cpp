...
static bool check_y_endstop = true;
...
void enable_y_endstop(bool check)
{
  check_y_endstop = check;
}
...
#ifdef MAD
//E axis Freewheeling interrupt
ISR(TIMER1_COMPB_vect)
{
//	SERIAL_ECHO("Freewheelin'");
//	if (READ(PSU_STALL_PIN) == 0) return;

	if (psu.enable == PSU_DISABLE) {
		ENABLE_STEPPER_DRIVER_INTERRUPT();
		DISABLE_FREEWHEEL_INTERRUPT();
		return;
	}
	if (READ(PSU_CONTACT_PIN) ==0)
	{
		WRITE(PSU_FIRING_PIN, LOW);
		//return;
	}
	if (READ(PSU_CONTACT_PIN) == 1) {
		WRITE(PSU_FIRING_PIN, HIGH);
	}

	if (READ(PSU_STALL_PIN) == 1) {
		ENABLE_STEPPER_DRIVER_INTERRUPT();
		DISABLE_FREEWHEEL_INTERRUPT();
		psu.contact = PSU_MAKE;
	}
	else{
//		WRITE(PSU_FIRING_PIN, LOW);
                NORM_E_DIR();
		WRITE_E_STEP(!INVERT_E_STEP_PIN);
		//SERIAL_ECHOLN("Freewheeling!");
		WRITE_E_STEP(INVERT_E_STEP_PIN);
	}
}
#endif
...
//	  #ifdef MAD
//	    stm32_spi_send_pay(current_block->gcode_line_N, current_block->psu_volt, current_block->psu_window, psu.enable);
//	  #endif
...
#ifdef MAD
	  if (psu.enable==PSU_ENABLE && ((out_bits & (1<<E_AXIS)) == 0) && (current_block->steps_e > 0)) {

		  if (READ(PSU_CONTACT_PIN) == 0) {
		  psu.contact = PSU_BREAK;
		  WRITE(PSU_FIRING_PIN, LOW);
		  DISABLE_STEPPER_DRIVER_INTERRUPT();
		  ENABLE_FREEWHEEL_INTERRUPT();
//		  SERIAL_ECHO("Freewheel enabled cont");
		  return;
		  }


		  if (READ(PSU_STALL_PIN) == 0) {
			WRITE(PSU_FIRING_PIN, HIGH);
			DISABLE_STEPPER_DRIVER_INTERRUPT();
			ENABLE_FREEWHEEL_INTERRUPT();
//			SERIAL_ECHO("Freewheel enabled stall");
			return;
		  }
		  else{
		  WRITE(PSU_FIRING_PIN, HIGH);
    ...
    if ((((out_bits & (1<<X_AXIS)) != 0)&&(out_bits & (1<<Y_AXIS)) == 0)) {   // -Y occurs for -A and +B
      ...
      CHECK_ENDSTOPS
      {
          ...
          if(y_max_endstop && old_y_max_endstop && (current_block->steps_y > 0) && check_y_endstop){
            endstops_trigsteps[Y_AXIS] = count_position[Y_AXIS];
            endstop_y_hit=true;
            step_events_completed = current_block->step_event_count;
          }
